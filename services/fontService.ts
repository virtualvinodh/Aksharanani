...
const createFont = (
    ...
    const nameToCharacterMap = new Map<string, Character>();
    characterSets.forEach(set => {
        set.characters.forEach(char => {
            allCharactersMap.set(char.unicode, char);
            nameToCharacterMap.set(char.name, char);
        });
    });

    const finalGlyphData = new Map(glyphData); // Mutable copy

    // Find the highest used PUA codepoint to start incrementing from there.
    // PUA ranges: U+E000..U+F8FF
    let puaCounter = 0xE000 - 1;
    allCharactersMap.forEach(char => {
    ...
    // Iterate through font rules to find ligatures that are not explicitly defined
    // in character sets, and assign them a PUA codepoint and composite glyph data.
    if (fontRules) {
        for (const scriptTag in fontRules) {
            if (!Object.prototype.hasOwnProperty.call(fontRules, scriptTag)) continue;
            ...
            for (const featureTag in scriptData) {
                ...
                if (featureData.liga) {
                    const rules = featureData.liga;
                    for (const ligName in rules) {
                        if (Object.prototype.hasOwnProperty.call(rules, ligName) && !nameToCharacterMap.has(ligName)) {
                            // This ligature needs to be dynamically created.
                            puaCounter++;
                            const componentNames = rules[ligName];

                            if (!Array.isArray(componentNames)) {
                                console.warn(`Skipping invalid ligature rule for '${ligName}' during PUA prefill: value is not an array.`);
                                continue;
                            }
                            
                            // Create a new Character definition for the ligature.
                            const newCharDef: Character = {
                                name: ligName,
                                unicode: puaCounter,
                                glyphClass: 'ligature',
                            ...
                            allCharactersMap.set(newCharDef.unicode, newCharDef);
                            ...
                            for (const componentName of componentNames) {
                                ...
                                if (componentChar) {
                                    ...
                                    if (isGlyphDrawn(componentGlyphData)) {
                                        ...
                                        // A component is defined but not drawn.
                                        prefillSuccessful = false;
                                        break;
                                    }
                                } else {
                                    // A component character doesn't exist in the definitions.
                                    prefillSuccessful = false;
                                    break;
                                }
                            }
                            ...
                            if (prefillSuccessful) {
                                ...
                                finalGlyphData.set(puaCounter, { paths: finalPaths });
                            }
                        }
                    }
                }
